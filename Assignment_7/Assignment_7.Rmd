...
Impute <- chr17 %>%
   filter (V8 == "Imputed") #filtering on value imputed to identify all records imputed

Impute <- count(Impute) #saving as a new variable the number of records where 'imputed'

Genotype <- chr17 %>%
   filter (V8 == "Genotyped")  #filtering on value imputed to identify all records genotyped
...
PoorImpute <- chr17 %>%
   filter (V8 == "Imputed", V7 <= 0.3) #filtering on value imputed to identify all records poorly imputed
...
The percentage of poorly imputed SNPs, ie at a value of < 0.3 is str(PerPoorImpute)
