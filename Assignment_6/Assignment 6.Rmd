...
```{r}
sample_reads_coverage$ID [which.max(x$read_coverage)]
  
```

### `#?#` *Which sample has the least? - 0.5 pt*
```{r}
sample_reads_coverage$ID [which.min(x$read_coverage)]
```
...
unique(HighCpm <- CpmBiConMelt %>%
  filter(value > 400) %>%
    pull(Region))
```
Of the 7 regions that have a CPM value greater than 400, 5 of them are in the Mitochondrial chromosomes.
...
ggplot(allDEStatsPairedTreatControlvsProtac_loess, aes(x = logCPM, y = logFC)) +
  geom_point() + geom_smooth(method = "loess", se = F) + geom_hline(yintercept = 0, color = "red")
...
BiConFilter_CPMlonger <- BiConFilter[,-1] %>% 
  cpm() %>% 
  as_tibble() %>% 
...
ggplot(BiConFilter_CPMlonger, aes(x = GC, y = value)) + 
  geom_point(alpha = 0.5) + geom_smooth(method = "gam", se = F) + 
  facet_wrap(~Sample) + ylim(0,50)

```

### `#?#` *Repeat the above, but this time showing only the lines of best fit and all on the same plot - 2 pt*
```{r}
ggplot(BiConFilter_CPMlonger, aes(x = GC, y = value, color = Sample)) + geom_smooth(method = "gam", se = F) + ylim(0,50)
```
...
Norm <- left_join(allDEStatsPairedTreatControlvsProtac_loess, data.frame("region" = names(gen_ranges), "GC" = gen_ranges$GC), 
            by = "region")

ggplot(Norm, aes(x = GC, y = logFC)) + geom_smooth(method = "gam", color = "blue") + geom_hline(yintercept = 0, color = "red")
...
```{r}
nonNorm <- left_join(allDEStatsPairedTreatControlvsProtac, data.frame("region" = names(gen_ranges), "GC" = gen_ranges$GC), 
            by = "region")

ggplot(nonNorm, aes(x = GC, y = logFC)) + geom_smooth(method = "gam", color = "blue") + geom_hline(yintercept = 0, color = "red")
```
...
```{r}
diff_peaks <- allDEStatsPairedTreatControlvsProtac %>% filter(FDR < 0.01)

```
`r nrow(diff_peaks)` differential peaks were observed.
